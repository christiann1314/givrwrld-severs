-- Clean up duplicate entries and add unique constraints
DELETE FROM public.plans WHERE id NOT IN (
    SELECT DISTINCT ON (slug) id FROM public.plans ORDER BY slug, created_at
);

-- Add unique constraints
ALTER TABLE public.games ADD CONSTRAINT games_slug_unique UNIQUE (slug) ON CONFLICT DO NOTHING;
ALTER TABLE public.plans ADD CONSTRAINT plans_slug_unique UNIQUE (slug) ON CONFLICT DO NOTHING;
ALTER TABLE public.bundles ADD CONSTRAINT bundles_slug_unique UNIQUE (slug) ON CONFLICT DO NOTHING;
ALTER TABLE public.addons ADD CONSTRAINT addons_slug_unique UNIQUE (slug) ON CONFLICT DO NOTHING;
