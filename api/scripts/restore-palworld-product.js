#!/usr/bin/env node
/**
 * Restore Palworld Product and Prices in Stripe
 * 
 * This script will:
 * 1. Check for Palworld plans in the database
 * 2. Create a Stripe product for Palworld (or use existing if found)
 * 3. Create prices for each Palworld plan
 * 4. Update the database with Stripe product and price IDs
 * 
 * Usage:
 *   node api/scripts/restore-palworld-product.js
 */
...
async function main() {
  console.log('🎮 Restore Palworld Product in Stripe');
  console.log('====================================\n');
  ...
  try {
    // Get all Palworld plans from database
    console.log('📋 Fetching Palworld plans from database...');
    let [plans] = await pool.execute(`
      SELECT id, display_name, game, ram_gb, vcores, ssd_gb, price_monthly, 
             stripe_price_id, stripe_product_id
      FROM plans
      WHERE game = 'palworld' AND is_active = 1
      ORDER BY ram_gb
    `);

    // If no plans exist, create them
    if (plans.length === 0) {
      console.log('⚠️  No Palworld plans found in database. Creating default plans...\n');
      
      const defaultPlans = [
        { id: 'palworld-4gb', ram_gb: 4, vcores: 2, ssd_gb: 40, price: 11.99, name: 'Palworld 4GB' },
        { id: 'palworld-8gb', ram_gb: 8, vcores: 3, ssd_gb: 80, price: 23.99, name: 'Palworld 8GB' },
        { id: 'palworld-16gb', ram_gb: 16, vcores: 4, ssd_gb: 160, price: 47.99, name: 'Palworld 16GB' }
      ];
    ...
    console.log(`✅ Found ${plans.length} Palworld plan(s):`);
    plans.forEach(plan => {
    ...
    if (!productId) {
      console.log('📦 Creating new Palworld product in Stripe...');
      const product = await stripe.products.create({
        name: 'Palworld Server',
        description: 'Palworld game server hosting with mod support',
        metadata: {
          game: 'palworld',
          type: 'game_server'
        }
